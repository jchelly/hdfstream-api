...
/*
  ...
  This version throws in a few requests for non-existent files and datasets.
*/
...
static void *read_dataset(void *data) {
  ...
  for(int i=0; i<nr_reads; i+=1) {
    ...
    struct ndarray res = receive_ndarray(tdata->hs, filename, datasetname, rank, start, count, buffer_size);

    /* Read should succeed if (and only if) the file and dataset exist */
    if((dataset_nr < nr_datasets) && (file_nr < nr_files)) {
      verify(res.status==0);
    } else {
      verify(res.status!=0);
    }
...
int main(int argc, char *argv[]) {
  ...
  /*
    Check that cache stats are consistent between manager process and workers

    This will fail if cache entries expire during the test because the count
    on the manager process doesn't take expiry into account.
  */
  for(int i=0; i<nr_processes; i+=1) {
    struct cache_info ci = hdfstream_cache_info(hs, i);
    struct reader_data *rd = hs->pool->worker[i]->data;
    struct shared_reader_data *srd = ((struct shared_reader_data *) rd->sm->data);
    verify(ci.nr_file_cache_hits == rd->file_cache->nr_cache_hits);
    verify(ci.nr_file_cache_misses == rd->file_cache->nr_cache_misses);
    printf("Process %d: cache hits %d, misses %d\n", i, srd->nr_file_cache_hits, srd->nr_file_cache_misses);
  }
