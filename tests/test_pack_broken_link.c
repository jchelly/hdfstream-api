...
/*
  Try packing a broken link - should be ok if we don't try to dereference it
*/
int main(int argc, char *argv[]) {
  ...
  /* Create a broken soft link */
  verify(H5Lcreate_soft("NoSuchGroup", file_id, "BrokenLink", H5P_DEFAULT, H5P_DEFAULT) >= 0);
  ...
  H5Lget_info(file_id, "BrokenLink", &link_info, H5P_DEFAULT);
  verify(pack_soft_link(file_id, "BrokenLink", *pk, &link_info) == 0);
  ...
  /* First entry should be "target" : "NoSuchGroup" */
  ...
  verify(strncmp(obj.via.map.ptr[1].val.via.str.ptr, "NoSuchGroup", obj.via.map.ptr[1].val.via.str.size) == 0);
