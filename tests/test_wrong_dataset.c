...
int main(int argc, char *argv[]) {
  ...
  /*
    Try to read a nonexistent dataset.
    This is checking that we can recover from a failed hdfstream_dataset_slice_open().
  */
  char wrong_datasetname[] = "abc";
  hsize_t start[] = {0};
  hsize_t count[] = {dims[0]};
  size_t buffer_size = 1024;
  struct ndarray res = receive_ndarray(hs, filename, wrong_datasetname, rank, start, count, buffer_size);
  verify(res.status!=0);

  /* Now try again with the correct name */
