...
/*
  This test checks that we can terminate a data stream early and still run
  subsequent requests successfully.
*/
...
int main(int argc, char *argv[]) {
  ...
  struct data_stream *stream = hdfstream_dataset_slice_open(hs, filename, datasetname,
                                                            rank, start, count, buffer_size);

  /* Allocate read buffer */
  char *buffer = malloc(buffer_size);

  /* Read several data blocks, but not the full stream (buffer size is much smaller than dataset) */
  for(int i=0; i<3; i+=1) {
    int status;
    verify(hdfstream_read_chunk(stream, buffer, &status) > 0);
  }
  verify(stream->ended==0);

  /* Abort the read early */
