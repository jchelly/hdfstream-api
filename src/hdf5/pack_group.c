...
int pack_group(hid_t obj_id, msgpack_packer pk, int max_depth,
  ...
  /* Get metadata for group members and count how many we can encode */
  int nr_members = 0;
  ...
  for(int link_nr=0; link_nr<nr_links; link_nr+=1) {
    ...
    case H5L_TYPE_EXTERNAL:
      ...
      /* Objects which are not groups or datasets will be skipped */
      if((oinfo[link_nr].type == H5O_TYPE_GROUP) || (oinfo[link_nr].type == H5O_TYPE_DATASET))nr_members += 1;
      break;
    case H5L_TYPE_SOFT:
  ...
  check(msgpack_pack_map(&pk, nr_members));
  for(int link_nr=0; link_nr<nr_links; link_nr+=1) {
